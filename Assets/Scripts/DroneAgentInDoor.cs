...
public class DroneAgentInDoor : Agent {
	float planeSize = 30f;
    float planeW = 10;
    float planeL = 30;
    ...
    void resetTarget(){
        goal.localPosition = new Vector3(Random.Range(-10, 10), 0.5f, Random.Range(3, 25));
        Target.localPosition = new Vector3(goal.localPosition.x + Random.value * 0.5f, 3, goal.localPosition.z + Random.value * 0.5f);
        for (int i = 0; i < Block.Length; i++){
            Block[i].localPosition = new Vector3(Random.Range(-13, 13),0,Random.value * 28);
            Block[i].rotation = Quaternion.Euler(new Vector3(-90, 0, Random.value * 360));
        }
