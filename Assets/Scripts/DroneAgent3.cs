...
public class DroneAgent3 : Agent {
    ...
    void resetTarget(){
        Target.localPosition = new Vector3(Random.value * planeW - 17, Random.value * 10 + 5, planeL-20);
        //Target.localPosition = new Vector3(Random.Range(-30, 30), Random.value * 15 + 5, Random.value * 50 + 30);
        for (int i = 0; i < Block.Length; i++){
            //Block[i].localScale = new Vector3(Random.value * 10 + 3, Random.value * 300, Random.value * 10 + 3);
            Block[i].localPosition = new Vector3(Random.Range(-35, 35), Block[i].localScale.y/2,Random.value * 60 + 20);
